...
export class UsersComponent implements OnDestroy,OnInit {
  ...
  constructor( public uiService: UiService, private searchService: SearchService,private router: Router) {
  ...
  getUsers(){
    this.searchService.getUsers(this.text,this.current_page,this.perPage).subscribe(response=>{
  ...
  searchInfo(login: string){
       this.searchService.getUserInfo(login).subscribe(val=>{ 
        ...
        this.uiService.sendDescription(val.bio ? val.bio : "This profile has no bio")
        this.uiService.sendTwitter( val.twitter_username? val.twitter_username : "Nil")
